'''
Train ResNet18 with a DETR-style transformer head on the COCO individuation task, using only bounding box information and no category labels.
...
# these match the cornet-z individuation training notebook settings
...
config = {
    ...
    # where to save the model checkpoints and config
    "output_path": "/zpool/vladlab/data_drive/geogaze_data/cornet_coco_bboxes/resnet18/individuation_critical",
...
# this starts from random weights, not imagenet pretrained weights
...
# %%
# full resnet18 individuation model
# resnet18 gives the last spatial feature map
# the transformer head predicts object/no object and boxes
class ResNet18DETRIndividuation(nn.Module):
    def __init__(
        self,
        num_queries=10,
        d_model=256,
        ...
        # get the last spatial feature map from resnet18
        self.backbone = ResNet18SpatialBackbone()
        # this gets made during the first forward pass
        # we need to see the resnet feature size before making the projection layer
        self.proj = None
        ...
        # transformer decoder
        # the object queries attend to the resnet feature map
    ...
    def _ensure_proj(self, feat):
        # make the 1x1 projection once we know how many resnet channels there are
    ...
    def forward(self, x):
        # get resnet feature map
        feat = self.backbone(x)
        # project resnet features into transformer dimension
        ...
        # each query attends to the resnet feature tokens
        hs = self.decoder(tgt=q, memory=src_tokens + pos_tokens)
        # predict object/no object and box for each query
...
# this version uses objectness and box quality for matching
...
# resnet18 gives the last spatial feature map, then the transformer head predicts objectness and boxes

model = ResNet18DETRIndividuation(
...
# %%
# set up the loss
# this does matching, object/no object loss, bbox loss, and giou loss
...
# %%
# set up optimizer
# use one learning rate for resnet18 and one for the transformer head
backbone_params = []
head_params = []
